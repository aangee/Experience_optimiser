class Asteroid {
    ...
    Update() {
        // v3: this.angle est déjà en radians (Math.random() entre -1 et 1).
        // En v2, la conversion /Math.PI*180 produisait des degrés → directions incorrectes.
    ...
    Draw() {
        ...
        // v3: même fix que dans Update() — this.angle est déjà en radians
        for (let i = 0; i < 12; i++) {
            this.#ctx.lineTo(
                this.x - (this.radius * this.seed[i]) * Math.cos(vertAngle * i + this.angle),
                this.y - (this.radius * this.seed[i]) * Math.sin(vertAngle * i + this.angle));
        }
        this.#ctx.closePath();
        //this.#ctx.fill();
        this.#ctx.stroke();
